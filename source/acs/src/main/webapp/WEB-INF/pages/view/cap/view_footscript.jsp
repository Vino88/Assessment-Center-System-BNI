...
function showExample()
{
	...
	$(document).ready(function() {
		...
		$('#wizard_cap').smartWizard({
			...
			<c:if test="${not empty listParticipantCap[6].lastTimer}">
			enableAllSteps: true,
			</c:if>
			...
			onFinish : function()
			{
				...
				alertify.confirm('<p>Sistem akan mengunci jawaban Anda</p> <p>Anda tidak bisa mengubah kembali jawaban Anda</p> <p>Anda akan diarahkan ke halaman <i>Leadership Inventory</i></p> <p> Apakah Anda yakin akan mengirimkan jawaban Anda sekarang ?</p>', function (e) {
				    if (e) {
				    	tinyMCE.triggerSave();
				    	$("body").loading();
				    	$("#capForm").attr("action", "${pageContext.request.contextPath}/controller/pages/view/cap/finish");
				    	$("#capForm").submit();
				    } else {
				        // user clicked "cancel"
				    }
		...
		$('#timer').tinyTimer({ 
			...
			onTick: function (val) {
				...
				if($('#wizard_cap').smartWizard("currentStep") == 1 && val.M == 30 && val.s == 0)
				{
					alertify.error("Anda terlalu lama di bagian satu.");
				}else if($('#wizard_cap').smartWizard("currentStep") == 2 && val.M == 20 && val.s == 0)
				{
					alertify.error("Anda belum menjawab pertanyaan pada bagian tiga.");
				}
				myToast.update({
					heading: 'Waktu tersedia',
				    text: $('#timer').text(),
				...
				if(val.s == 30 && val.m % 5 == 0)
				{
			    	...
			    	else
		    		{
			    		...
			    		if(moment().minute() != GLOBAL_LAST_SAVE.minute() )
			    		{
			    			tinyMCE.triggerSave();
				    		$("#GLOBAL_FLAG_DISABLED_SAVE").val("true");
				    		$.ajax({
