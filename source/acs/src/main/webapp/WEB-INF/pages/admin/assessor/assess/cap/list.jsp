                    ...
                    <form method="POST" id="demo-form2" data-parsley-validate="" class="form-horizontal form-label-left">
                       <div class="form-group">
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
					 	...
					 	<c:forEach items="${listParticipantCap}" var="dto">
					 		<div class="form-group">
								<label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">PK - ${dto.cap.number} <span class="required red">*</span></label>
								<div class="col-md-6 col-sm-6 col-xs-12">
								
									<c:choose>
										<c:when test="${dto.finished}">
											<a href="${pageContext.request.contextPath}/controller/pages/admin/assessor/assess/cap/capdetail/${participant.id}/${dto.cap.id}" class="btn btn-success form-control col-md-7 col-xs-12">PK - ${dto.cap.number}</a>
										</c:when>
										<c:otherwise>
											<c:set var="finishAll" scope="request" value="false" />
											<a href="${pageContext.request.contextPath}/controller/pages/admin/assessor/assess/cap/capdetail/${participant.id}/${dto.cap.id}" class="btn btn-warning form-control col-md-7 col-xs-12">PK - ${dto.cap.number}</a>
										</c:otherwise>
									</c:choose>
