					 	...
					 	<c:forEach items="${listParticipantCompetencyResultDisplayDto}" var="dto">
					 		<div class="form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									...
									<c:choose>
										<c:when test="${not empty dto.participantCompetencyLibraryFinalResult && dto.participantCompetencyLibraryFinalResult.inbasketRating > 0 }">
											<a href="${pageContext.request.contextPath}/controller/pages/admin/assessor/assess/simulation/competency/${participant.id}/${dto.competencyLibrary.id}" class="btn btn-success form-control col-md-7 col-xs-12">${dto.competencyLibrary.competencyName}</a>
										</c:when>
										<c:otherwise>
											<c:set var="finishAll" scope="request" value="false" />
											<a href="${pageContext.request.contextPath}/controller/pages/admin/assessor/assess/simulation/competency/${participant.id}/${dto.competencyLibrary.id}" class="btn btn-warning form-control col-md-7 col-xs-12">${dto.competencyLibrary.competencyName}</a>
										</c:otherwise>
									</c:choose>
