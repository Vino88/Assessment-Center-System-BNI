...
@WebFilter(filterName = "aliasloginfilter", urlPatterns = {"/controller/pages/alias/*"})
public class AliasLoginFilter extends AbstractFilter {
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException
	{
		HttpServletRequest httpRequest = (HttpServletRequest) request;
		HttpServletResponse httpResponse = (HttpServletResponse) response;

		if (httpRequest.getSession().getAttribute(	SessionConstants.ALIAS) != null)
		{
		...
		else
		{
			...
			httpResponse.sendRedirect(httpRequest.getContextPath() + "/controller/pages/open/auth/alias/login");
		}
	}
}
